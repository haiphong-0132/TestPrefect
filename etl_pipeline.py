...
try:
    ...
    raise RuntimeError(f"Failed to create connection string: {e}")
...
def extract_from_google_sheet(sheet_url: str):
    try:
        ...
        raise RuntimeError(f"Failed to extract data from Google Sheet: {e}")
...
def load_to_sql_server(df: pd.DataFrame, table_name: str):
    try:
        ...
        raise RuntimeError(f"Failed to load data to SQL Server: {e}")
